...
module diagonal_rod(grip = true) {
	for (y=[-1,1]) translate([0,40*y,0]) rotate([(90-vert_angle)*y,0,0]) translate([0,0,-50]) cylinder (h=100,r=4.2);
}

module diagonal_rod_cube (slide = 0, size = [16,16,10]) {
	for (y=[-1,1]) translate([0,40*y,0]) rotate([(90-vert_angle)*y,0,0]) translate([0,0,slide]) cube(size=size,center=true);
}

//module diagonal_rod_cover (obj, slide = 0) {
//	for (y=[-1,1]) translate([0,43*y,0]) rotate([(90-vert_angle)*y,0,0]) translate([0,0,slide]) obj;
//}
